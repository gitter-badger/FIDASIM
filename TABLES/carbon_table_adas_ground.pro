pro carbon_table_adas_ground
  ...
  qi=replicate(0.,nmax+1,nmax,neb,nti) 
  for ie=0,neb-1 do begin
     ...
     for iti=0L,nti-1 do begin  
	   ...
	   if n eq 0 then begin
             qi[nmax,n,ie,iti]= $
	       ;sigmav_ciionization_adas_ground(ti/float(aimp),eb/float(ab),n+1)
                beam_therm_rate(ti,eb,aimp,ab,de $
                                ,'sigma_ciionization_adas_ground',param=[n+1])
	   endif		      
        endfor
     endfor
  endfor
  
  file='qctable_adas_ground.bin'
