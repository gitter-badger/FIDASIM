...
!
  real*8,SAVE,dimension(4) :: axis_param_coldTarget !axis parameters(E[KeV/amu]) for table: 
                                                !min, max values, number of points,
                                                !specification 0 for LOG, 1 for LIN
  integer,SAVE :: npts                               ! number of points for table
  real*8,SAVE, dimension(:,:),allocatable :: sigv_table_prev ! tabulated sig*v  
  character*240, SAVE :: filename_prev(2)=(/'NONE','NONE'/)   ! filename from prev call
  integer, SAVE :: npts_prev(2)
...
!          write(*,*) "ADAS: Error during initialisation of splines."
       ...
       do isp = 1,N
          ...
          call UFILNAM('ADASDIR',adas_data_read,filename)
...
!    
    ...
    do iz=1,n_use
       ...
       adas_tables_file=trim('tables/sv/ex')//'_'//&
            & trim(adjustl(cizneut))//'_'//trim(adjustl(cizion))//'.cdf'
       !
       call UFILNAM('ADASDIR',adas_tables_file,filename)
       !
       ! try to open filename in tables directory
       call cdf_open(icdf,trim(filename),'r',fErr)
       if (fErr.ne.0) then !no table has been created
          write(6,*)' %adas_bms3d: OK table data being recomputed'
          ...
          npts_d=npts_d_param
          npts_t=npts_t_param
          CALL uf_read_3d(species_list(izi(iz)), e_vec, npts_e, d_vec, npts_d, t_vec, npts_t, sigv_table, iErr)
          !compute axis
          e_vec= e_vec*0.001_R8 !convert to [KeV/amu]
          t_vec= t_vec*0.001_R8 !convert to [KeV/amu]
          sigv_table= sigv_table*1e-6_R8 ! convert to [m**3/sec]
!mg          npts_e_min=1
!mg temproraly to eliminate spike in data file
          npts_e_min=4
!mg
          call cdf_open(icdf,trim(filename),'w',fErr)
          call cdf_define(icdf,'energy_vec',e_vec(npts_e_min:npts_e)) 
          call cdf_define(icdf,'density_vec',d_vec(1:npts_d)) 
          call cdf_define(icdf,'temp_vec',t_vec(1:npts_t)) 
          call cdf_define(icdf,'sigv_excite',sigv_table(npts_e_min:npts_e,1:npts_d,1:npts_t)) 
          call cdf_write(icdf,'energy_vec',e_vec(npts_e_min:npts_e))
          call cdf_write(icdf,'density_vec',d_vec)
          call cdf_write(icdf,'temp_vec',t_vec)
          call cdf_write(icdf,'sigv_excite',sigv_table(npts_e_min:npts_e,1:npts_d,1:npts_t)) 
          call cdf_close(icdf)  
          deallocate(e_vec,t_vec,d_vec,sigv_table)
          call cdf_open(icdf,trim(filename),'r',fErr)
       endif
...
!    
    ...
    do iz=1,n_use
       ...
       call cdf_open(icdf,trim(filename),'r',fErr)
       if (fErr.ne.0) then !no table has been created
          write(6,*)' %adas_bms1d: OK table data being recomputed'
          ...
          sigv_table= sigv_table*1e-6_R8 ! convert to [m**3/sec]
!mg          npts_e_min=1
!mg temproraly to elimminate spike in data file
          npts_e_min=4
!mg
          call cdf_open(icdf,trim(filename),'w',fErr)
          call cdf_define(icdf,'energy_vec',e_vec(npts_e_min:npts_e)) 
...
!    
    ...
    do iz=1,n_use
       ...
       call cdf_open(icdf,trim(filename),'r',fErr)
       if (fErr.ne.0) then !no table has been created
          write(6,*)' %adas_bms2d: OK table data being recomputed'
          !read and write table 
          if(allocated(sigv_table))deallocate(sigv_table)
          allocate(sigv_table(npts_e,npts_d,npts_t))
          ! allocate  energy, density and temperature  vectors
...
! read data from archive (refer to ADAS adf21 file format documentation at [2])
      call UFILNAM('ADASDIR',adas_ion_data(Z1),filename)
      open(unit=tape,file=filename,action='read', iostat=fErr)
      if (fErr .ne. 0) then                       ! check if file could be opened
         iErr = 6
         return
...
1000  format(i5)
1001  format(13x,i2)
1002  format(6(1x,E9.3E2),/,6(1x,E9.3E2))
...
!                               5 = Impact ionization = single II + full II + non-neutralaizing CX
  ...
  do iz=1, n_use
     ...
     !
     adas_tables_file=trim('tables/')//trim(adjustl(creact_type(fadas_type)))//&
     ...
     if (fErr.ne.0) then !no table has been created
        write(6,*)' %adas_sig: OK table data being recomputed'
        
        !compute and write table         
        !open file and read axis parameters
        adas_tables_file=trim('tables/')//trim(adjustl(creact_type(fadas_type)))//&
             &'/'//trim(adjustl(creact_type(fadas_type)))//'_'//&
             & trim(adjustl(cizneut))//'_'//trim(adjustl(cizion))//'_'//'tableParameters.cdf'
        call UFILNAM('ADASDIR',adas_tables_file,filename_param)
        call cdf_open(icdf,trim(filename_param),'r',fErr)
        if (fErr.ne.0) then 
           write(6,*)' %adas_sig: OK trying to read dummy data'
           !tables have not been initialized, try to read dummy data
           call UFILNAM('ADASDIR',trim('tables/')//trim(adjustl(creact_type(fadas_type)))//&
                &'/'//'tableParameters.cdf',filename_param)
           call cdf_open(icdf,trim(filename_param),'r',fErr)
           if(ferr.gt.0) then
              write(6,*)' ?adas_sig: faild to read table data'
              istat = 10
              return
           endif
        endif
        ...
        call cdf_close(icdf)
        !
!        write(*,*)' %adas_sig computed table'
        call compute_axis(e_axis,axis_param_coldTarget)
        !     
        npts=int(axis_param_coldTarget(3))
        if(allocated(sig_table))deallocate(sig_table)
        allocate(sig_table(npts))
        if(allocated(sig_table_wrk))deallocate(sig_table_wrk)
        allocate(sig_table_wrk(npts))
        sig_table=0.0_R8
        sig_table_wrk=0.0_R8
        if(fadas_type.le.4) then
           call adas_ion(e_axis,npts,izneut,izi(iz),fadas_type,sig_table,ierr)
        elseif(fadas_type.eq.5) then !Impact ionization = single II + full II + non-neutralaizing CX
           fadas_II=1 ! single ionizataion
           sig_table_wrk=0.0_R8
           call adas_ion(e_axis,npts,izneut,izi(iz),fadas_II,sig_table_wrk,ierr)
           sig_table=sig_table+sig_table_wrk
           if(izi(iz).gt.1) then
              fadas_CX=3 ! converted to adas_ion routine for non-neutralizing CX 
              sig_table_wrk=0.0_R8
              call adas_ion(e_axis,npts,izneut,izi(iz),fadas_CX,sig_table_wrk,ierr)
              sig_table=sig_table+sig_table_wrk
              if(izneut.gt.1) then
                 fadas_II=2 ! converted to adas_ion routine  for full ionizataion
                 sig_table_wrk=0.0_R8
                 call adas_ion(e_axis,npts,izneut,izi(iz),fadas_II,sig_table_wrk,ierr)
                 sig_table=sig_table+sig_table_wrk
                 if(izi(iz).gt.izneut) then
                    fadas_CX=4 ! converted to adas_ion routine for non-neutralized CX 
                    sig_table_wrk=0.0_R8
                   call adas_ion(e_axis,npts,izneut,izi(iz),fadas_CX,sig_table_wrk,ierr)
                    sig_table=sig_table+sig_table_wrk
                 endif
        ...
        if(ierr.gt.0) return !do not write table
        call cdf_open(icdf,trim(filename),'w',fErr)
        call cdf_define(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:4)) 
        call cdf_define(icdf,'sig',sig_table(1:npts)) 
        call cdf_write(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:4))
        call cdf_write(icdf,'sig',sig_table(1:npts)) 
        call cdf_close(icdf)  
        deallocate(e_axis)
        call cdf_open(icdf,trim(filename),'r',fErr)
     endif
...
!
  ...
  character*240 :: filename, filename_param    ! filename
  real*8,SAVE,dimension(4,2) :: axis_param_coldTarget_prev !axis parameters(E[KeV/amu]) for table: 
                                                !min, max values, number of points,
                                                !specification 0 for LOG, 1 for LIN
!
!checking input data for availability
  ...
  do iz=1, n_use
     ...
     ! create table's filename
     !
     adas_tables_file=trim('tables/')//trim(react_type(freact_type))//&
          &'/'//trim(react_type(freact_type))//'_'//&
          & trim(adjustl(cizneut))//'_'//trim(adjustl(cizion))//'_'//'coldTarget.cdf'
     !
     call UFILNAM('ADASDIR',adas_tables_file,filename)
     if(trim(adjustl(filename_prev(iz))).ne.trim(adjustl(filename))) then
     !
     ! try to open filename in tables directory
     call cdf_open(icdf,trim(filename),'r',fErr)
     
     if (fErr.ne.0) then !no table has been created
        write(6,*)' %adas_sigv: OK table data being recomputed'
        
        !compute and write table         
        !open file and read axis parameters
        ilen=len(trim(filename))
        filename_param= filename(1:ilen-14)//trim('tableParameters.cdf')
        call cdf_open(icdf,trim(filename_param),'r',fErr)
        if (fErr.ne.0) then 
           write(6,*)' %adas_sigv: OK trying to read dummy data'
           !tables have not been initialized, try to read dummy data
           call UFILNAM('ADASDIR',trim('tables/')//trim(react_type(freact_type))//&
                &'/'//'tableParameters.cdf',filename_param)
           call cdf_open(icdf,trim(filename_param),'r',fErr)
           if(ferr.gt.0) then
              write(6,*)' !adas_sigv: faild to read table data'
              istat = 10
              return
           endif
        endif
        ...
        sigv_table=0.0_R8
        ! set reaction key
        if(freact_type.eq.1) then !CX
           fadas_CX=4 ! converted to adas_ion routine for neutralizing CX 
           call sigprep_adas_z(fadas_CX,izneut,izi(iz),ikey)
           do i=1,npts
              eevec=e_axis(i)
              sigv_table(i)=sigall_z(eevec,vrel)*vrel
           enddo
        elseif(freact_type.eq.2) then !II
           fadas_II=5 ! converted to adas_ion routine for total ionizataions
           call sigprep_adas_z(fadas_II,izneut,izi(iz),ikey)
           do i=1,npts
              eevec=e_axis(i)
              sigv_table(i)=sigall_z(eevec,vrel)*vrel
           enddo
        ...
        call cdf_open(icdf,trim(filename),'w',fErr)
        call cdf_define(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:4)) 
        call cdf_define(icdf,'sigv',sigv_table(1:npts)) 
        call cdf_write(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:4))
        call cdf_write(icdf,'sigv',sigv_table(1:npts)) 
        call cdf_close(icdf)  
        deallocate(e_axis)
        call cdf_open(icdf,trim(filename),'r',fErr)
     endif
     ...
     call cdf_read(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:idims(1))) 
     call cdf_close(icdf)
     filename_prev(iz)=filename
     axis_param_coldTarget_prev(:,iz)=axis_param_coldTarget(:)
     npts_prev(iz)=npts
     
     if(allocated(sigv_table_prev)) then
        if(size(sigv_table_prev,dim=2).ge.iz) then
           if(size(sigv_table_prev(:,iz)).lt.npts) then
              if(iz.eq.1) then
                 deallocate(sigv_table_prev)
                 allocate(sigv_table_prev(npts,n_use))
                 sigv_table_prev=0.0_r8
              else ! extend
                 call extend(iz,n_use)
              endif
           endif
        else
           call extend(iz,n_use)
     ...
     sigv_table_prev(1:npts,iz)=sigv_table(1:npts)
  else
     npts=npts_prev(iz)
     if(allocated(sigv_table)) then
        if(size(sigv_table).ne.npts) then
           deallocate(sigv_table)
           allocate(sigv_table(npts))
        endif
     else
        allocate(sigv_table(npts))
     endif
     sigv_table(1:npts)=sigv_table_prev(1:npts,iz)
     axis_param_coldTarget(:)=axis_param_coldTarget_prev(:,iz)
 endif
     !interpolate 1D table
     xlr=log(axis_param_coldTarget(2)/axis_param_coldTarget(1))
...
!checking input data for availability
  ...
  do iz=1,n_use
     ...
     !
     ! create table's filename
     adas_tables_file=trim('tables/sv/sv')//'_'//&
          & trim(adjustl(cizneut))//'_'//trim(adjustl(cizion))//'_'//'coldTarget.cdf'
     call UFILNAM('ADASDIR',adas_tables_file,filename)
     if(trim(adjustl(filename_prev(iz))).ne.trim(adjustl(filename))) then
     !
     ...
     call cdf_open(icdf,trim(filename),'r',fErr)
     if (fErr.ne.0) then !no table has been created
        write(6,*)' %adas_zstop_sigvz: OK table data being recomputed'
        ilen=len(trim(filename))
        filename_param= filename(1:ilen-14)//trim('tableParameters.cdf')
        call cdf_open(icdf,trim(filename_param),'r',fErr)
        if (fErr.ne.0) then 
           write(6,*)' %adas_zstop_sigvz: OK trying to read dummy data'
           !tables have not been initialized, try to read dummy data
           call UFILNAM('ADASDIR',trim('tables/sv/tableParameters.cdf'),filename_param)
           call cdf_open(icdf,trim(filename_param),'r',fErr)
           if(ferr.gt.0) then
              write(6,*)' !adas_zstop_sigvz: faild to read table data'
              istat = 10
              return
           endif
        endif
        ...
        if(allocated(sigv_table))deallocate(sigv_table)
        allocate(sigv_table(npts))
        if(allocated(sigv_table_wrk))deallocate(sigv_table_wrk)
        allocate(sigv_table_wrk(npts))
        sigv_table=0.0_R8
        sigv_table_wrk=0._R8
        freact_type=1 !CX reaction
        iopen_file=0 
        call adas_sigv(freact_type,e_axis,npts,izneut,dble(izi(iz)),sigv_table,istat)
        iopen_file=iopen_file+istat !iopen_file <4 means that tables are opened
        freact_type=2 !II reaction
        call adas_sigv(freact_type,e_axis,npts,izneut,dble(izi(iz)),sigv_table_wrk,istat)
        iopen_file=iopen_file+istat !iopen_file <4 means that tables are opened
        sigv_table=sigv_table+sigv_table_wrk !stopping rate= CX+II
        if(iopen_file.gt.3) then !errors appear in both part: CX and II. => sigv_table=0
           !do not open table
           sigv_adas=0._R8
           return
        endif
...
!checking input data for availability
  ...
  do iz=1,n_use
     ...
     !   
     if (fErr.ne.0) then !no table has been created
        write(6,*)' %adas_btsigv: OK table data being recomputed'
        !compute and write table         
        !open file and read axis parameters
        ilen=len(trim(filename))
        filename_param= filename(1:ilen-14)//trim('tableParameters.cdf')
        ...
        if (fErr.ne.0) then 
           !tables have not been initialized, try to read dummy data
           write(6,*)' %adas_btsigv: OK trying to read dummy data'
           call UFILNAM('ADASDIR',trim('tables/')//trim(react_type(freact_type))//&
                &'/'//'tableParameters.cdf',filename_param)
           call cdf_open(icdf,trim(filename_param),'r',fErr)
           if(ferr.gt.0) then
              write(6,*)' !adas_btsigv: faild to read table data'
              istat = 10
              return
           endif
        endif
        ...
        call cdf_close(icdf)
        !compute axis
!        write(*,*)'  %adas_btsigv : computing tables: ',trim(adas_tables_file)
        call compute_axis(e_axis,axis_param_warmTarget(1:4))
        call compute_axis(t_axis,axis_param_warmTarget(5:8))
        !     
        npts_e=int(axis_param_warmTarget(3))
        npts_t=int(axis_param_warmTarget(7))
        if(allocated(sigv_table))deallocate(sigv_table)
        ...
        if(freact_type.eq.1) then !CX
           fadas_CX=4 ! converted to adas_ion routine for neutralizing CX 
           call sigprep_adas_z(fadas_CX,izneut,izi(iz),ikey)
           call comptabl_z(wrk,sigv_table,e_axis,t_axis,npts_e,npts_t,ier)
           
        elseif(freact_type.eq.2) then !II
           fadas_II=5 ! converted to adas_ion routine for total ionizataion
           call sigprep_adas_z(fadas_II,izneut,izi(iz),ikey)
           call comptabl_z(wrk,sigv_table,e_axis,t_axis,npts_e,npts_t,ier)
        endif
        call cdf_open(icdf,trim(filename),'w',fErr)
        call cdf_define(icdf,'axis_param_warmTarget',axis_param_warmTarget(1:8)) 
        call cdf_define(icdf,'btsigv',sigv_table(1:npts_e,1:npts_t)) 
        call cdf_write(icdf,'axis_param_warmTarget',axis_param_warmTarget(1:8))
        call cdf_write(icdf,'btsigv',sigv_table(1:npts_e,1:npts_t)) 
        call cdf_close(icdf)  
        deallocate(e_axis,t_axis)
        call cdf_open(icdf,trim(filename),'r',fErr)
     endif
...
!check input data
  ...
  ! create table's filename
  !
  adas_tables_file=trim('tables/')//trim(react_type)//&
       &'/'//trim(react_type)//'_'//&
       & trim(adjustl(cizneut))//'_'//'coldTarget.cdf'
  !
  call UFILNAM('ADASDIR',adas_tables_file,filename)
  if(trim(adjustl(filename_prev(1))).ne.trim(adjustl(filename))) then
  !
  ! try to open filename in tables directory
  call cdf_open(icdf,trim(filename),'r',fErr)
  
  if (fErr.ne.0) then !no table has been created
     write(6,*)' %adas_sigvte_ioniz: OK table data being recomputed'
     !compute and write table         
     !open file and read axis parameters
     ilen=len(trim(filename))
     filename_param= filename(1:ilen-14)//trim('tableParameters.cdf')
     call cdf_open(icdf,trim(filename_param),'r',fErr)
     if (fErr.ne.0) then 
        write(6,*)' %adas_sigvte_ioniz: trying to read dummy data'
        !tables have not been initialized, try to read dummy data
        call UFILNAM('ADASDIR',trim('tables/')//trim(react_type)//&
             &'/'//'tableParameters.cdf',filename_param)
        call cdf_open(icdf,trim(filename_param),'r',fErr)
        if(ferr.gt.0) then
           write(6,*)' %adas_sigvte_ioniz: faild to read table data'
           istat = 10
           return
        endif
     endif
     ...
     call cdf_close(icdf)
     !compute axis
     call compute_axis(t_axis,axis_param_coldTarget)
     !     
     npts=int(axis_param_coldTarget(3))
     if(allocated(sigv_table))deallocate(sigv_table)
     allocate(sigv_table(npts))
     sigv_table=0.0_R8
     if(izneut.eq.1) then
        call adas_ei_h(t_axis,npts,sigv_table,ier)
     elseif(izneut.eq.2) then
        call adas_ei_he(t_axis,npts,sigv_table,ier)
     endif
...
!write table to file
  ...
  call cdf_read(icdf,'axis_param_coldTarget',axis_param_coldTarget(1:idims(1))) 
  call cdf_close(icdf)
  filename_prev(1)=filename
  
  if(allocated(sigv_table_prev)) then
     if(size(sigv_table_prev(:,1)).ne.npts) then
        deallocate(sigv_table_prev)
        allocate(sigv_table_prev(npts,1))
        sigv_table_prev(:,1)=0.0_r8
     endif
  else
     allocate(sigv_table_prev(npts,1))
     sigv_table_prev(:,1)=0.0_r8
  endif
  sigv_table_prev(1:npts,1)=sigv_table(1:npts)
  else
     if(allocated(sigv_table)) then
        if(size(sigv_table).ne.npts) then
           deallocate(sigv_table)
     ...
     sigv_table(1:npts)=sigv_table_prev(1:npts,1)
  endif
...
! read data from archive (refer to ADAS adf21 file format documentation at [2])
      call UFILNAM('ADASDIR',adas_ei_data,filename)
      open(unit=tape,file=filename,action='read', iostat=fErr)
...
!-----------------------------------------------------------------------------------------
!
SUBROUTINE uf_read_3d(extstr,x1r8,nx1,x2r8,nx2,x3r8,nx3,data_outr8,ierr)
implicit none

integer            :: nshot   = 1               ! version of ADAS 3D dataset
character (len=4)  :: idev    = 'ADAS'          ! 
character (len=40) :: udb_dir = 'data/bms/udb/'     ! directory which contains ufiles
!MG NOTE from December 02, 2011 !!!
!MG I verified and insist that Target temperature TI is supposed to be in [Kev]
!    not in [KeV/amu]. Files bms/udb/A00001* have an error in Ion Temperature defenition
!    it is not  eV/amu but eV.
!     Also note that these data bms/udb/A00001* have been created for regime when T_electron = T_ion
integer :: nx1, nx2, nx3
integer :: nx1max, nx2max, nx3max
real*8    :: x1r8(nx1), x2r8(nx2), x3r8(nx3), data_outr8(nx1,nx2,nx3)
real,dimension(:), allocatable   :: x1, x2, x3
real,dimension(:,:,:), allocatable   :: data_out
integer :: iochan, iproc, iflag, ier, ierr

character (len=1)  :: prestr='A'
character (len=4)  :: extstr
character (len=30) :: dlabstr,x1labstr,x2labstr,x3labstr

character (len=10) :: sdate
character (len=10) :: disk

integer, parameter :: nscalmax = 0
integer            :: nscal    = 0
real ,dimension(:), allocatable  :: valscal
character(len=30)  :: strscal(nscalmax)
character*240 :: dir_name
nx1max = nx1
nx2max = nx2
nx3max = nx3
...
call UFILNAM('ADASDIR',udb_dir,dir_name)
allocate(x1(nx1),x2(nx2),x3(nx3),data_out(nx1,nx2,nx3))
allocate(valscal(max(1,nscalmax)))
x1=0.0
x2=0.0
x3=0.0
data_out=0.0
...
call ufncap(2)
call ufsetr(iochan,prestr,extstr,disk,trim(dir_name))
call ufemsg(1)
call ufimsg(1)
call ufoprd(iochan,nshot,ier)
call uf3drd(iochan,idev,sdate,data_out,nx1,nx2,x1,nx1max,x2,nx2max,     &
     &      x3,nx3max,iproc,nx1,nx2,nx3,iflag,dlabstr,x1labstr,      &
     &      x2labstr,x3labstr,nscalmax,nscal,valscal,strscal,ierr)
call ufclos(iochan)
 
if (ierr.ne.0) then
   ierr = 5 !ADAS file reading error
else
   x1r8=dble(x1)
   x2r8=dble(x2)
   x3r8=dble(x3)
   data_outr8=dble(data_out)
endif
deallocate(x1,x2,x3,data_out,valscal)

return
end SUBROUTINE uf_read_3d

subroutine extend(i_z,nuse)
  implicit none
  integer :: i_z,nuse
  real*8, dimension(:),allocatable :: tmp! wrk.
  allocate(tmp(npts))
  tmp=0.0_r8
  tmp(1:npts_prev(i_z-1))=sigv_table_prev(1:npts_prev(i_z-1),i_z-1)
  deallocate(sigv_table_prev)
  allocate(sigv_table_prev(maxval(npts_prev(:)),nuse))
  sigv_table_prev=0.0_r8
  sigv_table_prev(1:npts_prev(i_z-1),i_z-1)=tmp(1:npts_prev(i_z-1))
  deallocate(tmp)
